                    ...
                    <div class="max-w-2xl mx-auto px-4 lg:max-w-none lg:px-0">
                        ...
                        <ul role="list" class="text-sm font-medium divide-y divide-white divide-opacity-10">
                            <li v-for="item in cart" :key="item.id" class="flex items-start py-6 space-x-4">
                                <img :src="item.image" :alt="item.name" class="flex-none w-20 h-20 rounded-md object-center object-cover" />
                                <div class="flex-auto space-y-1">
                                    <h3 class="text-white">{{ item.name }}</h3>
                                    <p>${{ item.price.toFixed(2) }} x {{ item.quantity }}</p>
                                </div>
                                <p class="flex-none text-base font-medium text-white">${{ (item.price*item.quantity).toFixed(2) }}</p>
                            </li>
                        </ul>
                        ...
                        <dl class="text-sm font-medium space-y-6 border-t border-white border-opacity-10 pt-6">
                            ...
                            <div class="mt-10 flex justify-end pt-6 border-t border-gray-200">
                                <button @click="checkout = true" v-if="cart_subtotal > 0" type="submit" class="bg-indigo-600 border border-transparent rounded-md shadow-sm py-2 px-4 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-indigo-500">Go to payment</button>
                            </div>
                        </dl>
                    </div>
                </section>
                <section v-if="checkout" aria-labelledby="payment-and-summary" class="py-16 lg:max-w-lg lg:w-full lg:mx-auto lg:pt-0 lg:pb-24 lg:row-start-1 lg:col-start-1 lg:col-span-2">
                    <form @submit.prevent="pay">
                        ...
                        <div class="max-w-2xl mx-auto px-4 lg:max-w-none lg:px-0">
                            ...
                            <div class="mt-10 flex justify-end pt-6 border-t border-gray-200">
                                <button @click="checkout = false" type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">Cancel</button>
                ...
                <section v-else-if="cart_order" aria-labelledby="payment-and-summary" class="py-16 lg:w-full lg:mx-auto lg:pt-0 lg:pb-24 lg:row-start-1 lg:col-start-1 lg:col-span-2">
                    <div class="max-w-2xl mx-auto py-16 px-4 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8 lg:py-32">
                        ...
                        <div class="mt-10 flex justify-start pt-6 border-t border-gray-200">
                            <button @click="newOrder" type="button" class="bg-indigo-600 border border-transparent rounded-md shadow-sm py-2 px-4 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-indigo-500">New order</button>
                        </div>
                    </div>
                </section>
                    ...
                    <div>
                        <div class="grid grid-cols-3 gap-x-2 sm:grid-cols-4 sm:gap-x-6">
                            <div v-for="(category, index) in menu" :key="category.id"
                                ...
                                <div class="p-2 sm:p-6 flex items-end">
                                    <div>
                                        <h3 class="text-white" :class="selectedCategory == index ? 'font-bold' : 'font-semibold'">
                        ...
                        <div v-if="selectedCategory >= 0" class="py-6">
                            <div class="grid grid-cols-3 gap-y-10 gap-x-2 sm:grid-cols-4 sm:gap-x-6">
                                <div v-for="item in menu[selectedCategory].items" :key="item.id" class="group cursor-pointer">
                                    <div
                                    ...
                                    <div class="mt-4 mb-2 flex items-center justify-between">
                                        ...
                                        <span v-if="cart[item.id]" class="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium bg-indigo-100 text-indigo-800">
                                            x
                                    ...
                                    <div class="flex flex-col items-start sm:flex-row sm:justify-between sm:items-center">
                                        ...
                                        <span class="relative z-0 inline-flex shadow-sm rounded-md">
                                            <button @click="removeItem(item)" type="button" class="relative inline-flex items-center px-2 py-1 rounded-l-md border border-gray-300 bg-white text-sm font-bold text-red-500 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                                              -
                                            </button>
                                            <button @click="addItem(item)" type="button" class="-ml-px relative inline-flex items-center px-2 py-1 rounded-r-md border border-gray-300 bg-white text-sm font-bold text-green-500 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                                              +
                                            </button>
                                          </span>
                                    </div>
